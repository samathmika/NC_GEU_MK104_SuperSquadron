...
public class Bakrawala<Signals> extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        final String sama = getIntent().getStringExtra("array");
        final String Signals[] = sama.split(",");
        //List<String> al = new ArrayList<>();
        //al = Arrays.asList(Signals);
        final int[] i = {0};

        new CountDownTimer(55000, 5000) {

            public void onTick(long millisUntilFinished) {
                if (Signals[i[0]].equals(" true") || Signals[i[0]].equals("[true") || Signals[i[0]].equals(" true]") )
                {
